class WaveDrawer {
  ...
  */
  ...
  private void drawSliderUI() {
    ...
    float handleX = map(highlightedIdx, 0, totalLines - 1, sliderX, sliderX + sliderWidth);
    ...
    text("Wavetable Index Slider: " + highlightedIdx, sliderX, sliderY - 25);
  }
  ...
  void checkSliderInteraction(float mx, float my) {
    ...
    if (mx >= sliderX - 10 && mx <= sliderX + sliderWidth + 10 && my >= sliderY - 20 && my <= sliderY + 20) {
      float rawNorm = (mx - sliderX) / sliderWidth;
